...
public class MovieSeatBooking  implements Runnable {
	...
	//movie ticket booking method implementation using threading concept
	public synchronized void movieBooking() {
		movie_list();//calling movie list method
		String movie_name;
		String date;
		int seats=100;
		...
		Collections.sort(movies,new MovieByName());//sorting operation performance using comparator
		MovieSeatBooking tr=new MovieSeatBooking(movie_name,null,0);
		int index=Collections.binarySearch(movies,tr,new MovieByName());//searching operation perform
		
		if(index<0)
			System.out.println("Movie is not availble");
		else
		{	//movie ticket printing
...
//sorting performance using comparator
